...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addUserOption(option =>
            option.setName('name')
            .setDescription('The name of the one to kick :)')
            .setRequired(true))
    ...
    async execute(interaction, client) {
        ...
        const buildEmbed = (toEmbed) => {
            const embed = new MessageEmbed()
                .setTitle(`:white_check_mark:  ${toEmbed.user.username} gekickt.`);
            return embed;
        }
        
        
        if(member.user.id == client.user.id) {
            await interaction.reply(`<@${interaction.user.id}> wollte mich kicken.`)
